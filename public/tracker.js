...
const REMINDER_LOG_KEY = "siteControlReminderLog_v1";
...
function saveReminderLog(log) {
  try {
    ...
    /* storage unavailable — reminders just won't persist across reloads */
  }
}
...
async function updateRow(id, payload) {
  try {
    ...
    delete reminderLog[id]; // stage/start-date may have changed the deadline — allow re-reminding
...
function checkReminders() {
  rows.forEach((row) => {
    ...
    if (remainSec > 0 && ratio <= REMINDER_RATIO) {
      const last = reminderLog[row.id];
      if (!last || Date.now() - last >= REMINDER_COOLDOWN_MS) {
        reminderLog[row.id] = Date.now();
        saveReminderLog(reminderLog);
